...
def Mom_generation(prompt):
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a helpful assistant that summarizes meeting transcripts into specific sections."
            },
            {
                "role": "user",
                "content": f"""From the following transcript, generate only the following sections in bullet points:
                        - Agenda
                        - Discussion Points
                        - Conclusion

                        Do not include Date, Time, Location, Participants, or Action Items.
